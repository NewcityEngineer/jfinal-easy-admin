...
public class BaiduWeatherService {
	...
	private static List<BaiduWeatherEntity> getBaiduWeather(String location) {
		...
		String url = "http://api.map.baidu.com/telematics/v3/weather?" + "location={location}" + "&output=json" + "&ak="
				+ ak;
		...
		if (error == 0 && status.equalsIgnoreCase("success")) {
			...
			for (int i = 0; i < weather_data.size(); i++) {
				...
				list.add(new BaiduWeatherEntity(currentCity, date, resdate, dayPictureUrl, nightPictureUrl, resweather,
						wind, temperature, pm25, other));
			}
		}
		return list;
	}
	...
	public static OutNewsMsg getWeatherService(String location, InTextMsg inTextMsg) {
		OutNewsMsg outMsg = new OutNewsMsg(inTextMsg);
		outMsg.setCreateTime((int) new Date().getTime());
		...
		if (entities.size() > 0) {
			for (BaiduWeatherEntity baiduWeatherEntity : entities) {
				...
				if (hour > 8 && hour < 20) {
					outMsg.addNews(
							baiduWeatherEntity.getResDate() + " 天气:" + baiduWeatherEntity.getWeather() + " 风力:"
									+ baiduWeatherEntity.getWind() + " 温度:" + baiduWeatherEntity.getTemperature(),
							"", baiduWeatherEntity.getDayPictureUrl(), "http://wx.cyansoft.cn/weather");
				} else {
					outMsg.addNews(
							baiduWeatherEntity.getResDate() + " 天气:" + baiduWeatherEntity.getWeather() + " 风力:"
									+ baiduWeatherEntity.getWind() + " 温度:" + baiduWeatherEntity.getTemperature(),
							"", baiduWeatherEntity.getNightPictureUrl(), "http://wx.cyansoft.cn/weather");
				}
			}

		} else {
			outMsg.addNews("没有找到对应的天气，请检查城市是否输入正确！！", "", "", "http://wx.cyansoft.cn/weather");
		}
